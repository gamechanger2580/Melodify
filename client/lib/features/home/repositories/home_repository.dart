...
class HomeRepository {
  Future<Either<AppFailure, String>> uploadSong({
    ...
    try {
      final request = http.MultipartRequest(
        'POST',
        Uri.parse('${ServerConstatnt.serverUrl}/song/upload'),
      );
  ...
  Future<Either<AppFailure, List<SongModel>>> getAllSongs({
    ...
    try {
      final res = await http.get(
        Uri.parse('${ServerConstatnt.serverUrl}/song/list'),
        headers: {
  ...
  Future<Either<AppFailure, bool>> favSong({
    ...
    try {
      final res = await http.post(
        Uri.parse('${ServerConstatnt.serverUrl}/song/favorite'),
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token,
        },
  ...
  Future<Either<AppFailure, List<SongModel>>> getFavSongs({
    ...
    try {
      final res = await http.get(
        Uri.parse('${ServerConstatnt.serverUrl}/song/list/favorites'),
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token,
        },
